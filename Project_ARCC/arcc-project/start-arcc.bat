...
echo ============================================
echo   Starting RCC - Resume ^& Career Coach
echo ============================================
echo.

REM Start MySQL database via Docker Compose
echo [1/4] Starting MySQL database via Docker...
docker compose stop database 2>nul
docker compose rm -f database 2>nul
...
start "RCC Backend" cmd /k cd /d "%ROOT%backend" ^&^& python -m pip install -r requirements.txt ^&^& python app.py

REM Frontend terminal (CALL is required for npm.cmd)
echo [4/4] Starting React frontend on port 3000...
start "RCC Frontend" cmd /k cd /d "%ROOT%frontend" ^&^& if not exist node_modules\ (call npm install) ^&^& set REACT_APP_API_BASE=http://localhost:5000/api ^&^& echo Starting React at http://localhost:3000 ^&^& call npm start
